...
## Current State — UI Fixes & Tối ưu bộ lọc (Nhánh feat/ui-fixes) — ĐÃ HOÀN THÀNH (04/06/2026)
- **Đã hoàn thành 3/3 TIPs sửa lỗi UI/UX và tối ưu bộ lọc**:
  - **TIP-fix-001 (Sửa Sidebar Active Menu)**: Đồng bộ lại trạng thái menu active trên sidebar khi chuyển đổi giữa các trang (bao gồm cả các trang mới tích hợp ở Phase 8 như `/bccp/new-customer`, `/bccp/retention`).
  - **TIP-fix-002 (Lọc bỏ HCC khỏi Cơ cấu BCCP)**: Thay đổi các câu lệnh query trong `revenue.py` sang kết nối với bảng `dim_dichvu` có chỉ mục và thêm điều kiện `WHERE (d.nhom_chinh = 'BCCP' OR d.nhom_chinh IS NULL)` để loại bỏ hoàn toàn phần doanh thu Hành chính công cũ khỏi các biểu đồ và chỉ số BCCP.
  - **TIP-fix-003 (Nút áp dụng bộ lọc)**: Thêm nút `"🔍 Áp dụng bộ lọc"` vào sidebar để người dùng kích hoạt lọc sau khi chọn xong, chuyển đổi các callback tính toán chính sang sử dụng `State` thay vì `Input` tự động trigger. Sửa logic `toggle_period_filters` sử dụng context để tránh mất giá trị ngày/tháng/năm đã chọn.
- **Trạng thái nhánh**: Sẵn sàng để Sếp chạy Dashboard nghiệm thu tại local và merge vào nhánh `main`.
...
- **Phase 5D** (Tính năng mới: Chi tiết Khách hàng CMS): 4 TIPs — `[x]` ĐÃ NGHIỆM THU ✅ (2026-05-31, 4/4 TIPs PASS)
...
10. **[DONE ✅] Phase 8 — Nâng cấp Module BCCP (retention, new customer, gộp UI)**:
    ...
    - [x] TIP-bccp-006 (P0): Trang KHHH Retention (`/bccp/retention`) ✅
     ...
     - **QA VERIFY**: PASS 7/7 TIPs — merged vào main ngày 04/06/2026
     - **Worktree**: đã xóa sau merge
11. **[PENDING] Nghiệm thu thực tế Phase 8**: Sếp chạy Dashboard và kiểm tra các trang mới:
    - `/bccp` — KPI + 3 biểu đồ
    - `/bccp/customer` — bộ lọc DV inline + bảng revenue
    - `/bccp/new-customer` — KH mới theo xã/cụm + so KH
    - `/bccp/retention` — KHHH retention rate + biến động
12. **[PENDING] Nghiệm thu thực tế nhánh feat/ui-fixes**: Sếp kiểm tra các lỗi giao diện và tối ưu bộ lọc:
...
13. **[PENDING] Phase 5C**: Deploy server nội bộ + chuyển PostgreSQL

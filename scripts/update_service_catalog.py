...
# Đường dẫn tính từ project_root (không hardcode worktree)
EXCEL_PATH = project_root / 'data' / 'dieu-chinh' / 'dieu_chinh_dichvu.xlsx'
CSV_OUT_PATH = project_root / 'data' / 'mapping-spdv.csv'
MA_REF_PATH = project_root / 'data' / 'mau-file-import' / 'ma_tham_chieu.xlsx'


def main():
    logger.info("=== BAT DAU CAP NHAT DANH MUC DICH VU & SINH MA THAM CHIEU ===")
    ...
    logger.info(f"Dang doc file Excel: {os.path.basename(EXCEL_PATH)}...")
    ...
    logger.info(f"Tim thay {len(df_new)} dong dich vu can cap nhat.")
    ...
    try:
        ...
        logger.info("Dang xoa cac dong danh muc cu cua HCC, TCBC, PPBL trong dim_dichvu...")
        cursor.execute("DELETE FROM dim_dichvu WHERE nhom_chinh IN ('HCC', 'TCBC', 'PPBL')")
        logger.info(f"Da xoa {cursor.rowcount} dong danh muc cu.")

        # Chèn danh mục mới
        logger.info("Dang nap danh muc moi vao dim_dichvu...")
        inserted_count = 0
        ...
        conn.commit()
        logger.info(f"Nap thanh cong {inserted_count} dong danh muc moi vao dim_dichvu.")

        # 3. Xuất ra file CSV mapping để đồng bộ Git tracking
        logger.info(f"Dang doc lai toan bo dim_dichvu de xuat ra CSV...")
        ...
        logger.info(f"Da xuat va dong bo file mapping tai: {CSV_OUT_PATH}")

        # 4. Sinh lại file ma_tham_chieu.xlsx với 3 sheet
        logger.info(f"Dang sinh lai file ma_tham_chieu.xlsx tai: {MA_REF_PATH}...")
        ...
        MA_REF_PATH.parent.mkdir(parents=True, exist_ok=True)
        ...
        logger.info("Da sinh va cap nhat thanh cong ma_tham_chieu.xlsx voi 3 sheet moi nhat.")
    ...
    logger.info("=== HOAN THANH CAP NHAT DANH MUC DICH VU & SINH MA THAM CHIEU ===")

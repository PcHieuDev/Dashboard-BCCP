...
def main():
    print("=== BAT DAU CAP NHAT DANH MUC DICH VU & SINH MA THAM CHIEU ===")
    
    if not os.path.exists(EXCEL_PATH):
        print(f"Loi: Khong tim thay file Excel tai {EXCEL_PATH}")
        return
        
    # 1. Đọc file Excel điều chỉnh
    print(f"Dang doc file Excel: {os.path.basename(EXCEL_PATH)}...")
    ...
    print(f"Tim thay {len(df_new)} dong dich vu can cap nhat.")
    ...
    try:
        ...
        print("Dang xoa cac dong danh muc cu cua HCC, TCBC, PPBL trong dim_dichvu...")
        cursor.execute("DELETE FROM dim_dichvu WHERE nhom_chinh IN ('HCC', 'TCBC', 'PPBL')")
        print(f"Da xoa {cursor.rowcount} dong danh muc cu.")
        
        # Chèn danh mục mới
        print("Dang nap danh muc moi vao dim_dichvu...")
        inserted_count = 0
        ...
        conn.commit()
        print(f"Nap thanh cong {inserted_count} dong danh muc moi vao dim_dichvu.")
        
        # 3. Xuất ra file CSV mapping để đồng bộ Git tracking
        print(f"Dang doc lai toan bo dim_dichvu de xuat ra CSV...")
        ...
        print(f"Da xuat va dong bo file mapping tai: {CSV_OUT_PATH}")
        
        # 4. Sinh lại file ma_tham_chieu.xlsx với 3 sheet
        print(f"Dang sinh lai file ma_tham_chieu.xlsx tai: {MA_REF_PATH}...")
        ...
        print("Da sinh va cap nhat thanh cong ma_tham_chieu.xlsx voi 3 sheet moi nhat.")
    ...
    print("=== HOAN THANH CAP NHAT DANH MUC DICH VU & SINH MA THAM CHIEU ===")
